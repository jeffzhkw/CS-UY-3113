...
void Map::Build(){
    for (int y = 0; y < this->height; y++){//row
        for(int x = 0; x < this->width; x++){// column
            
            int tile = levelData[y * width + x];//containing all the textureID
            if (tile == 0) continue;
        
            float u = (float)(tile % tile_count_x) / (float)tile_count_x;//calculate uv coord
            float v = (float)(tile / tile_count_x) / (float)tile_count_y;
            
            float tileWidth = 1.0f/(float)tile_count_x;//in terms of uv coord;
            float tileHeight = 1.0f/(float)tile_count_y;
            
            float xoffset = -(tile_size/2);
            float yoffset = (tile_size/2);
            
            vertices.insert(vertices.end(), {//init for drawing out
